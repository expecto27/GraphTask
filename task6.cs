...
namespace Graph_tasks
{
    public partial class task6 : Form
    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            if (k == FindNumberOfComponents(matrix))
            {
                MessageBox.Show("Поздравляем", "Все верно", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
            }
            else
            {
                MessageBox.Show("Поздравляем", "Не верно", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
